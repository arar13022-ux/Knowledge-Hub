...
export default function FeedbackPage() {
  ...
  // Unpublish: just hides the article, doesn't resolve the report
  ...
  // Resolve: republishes the article (if needed) AND clears every outdated report for it.
  // Used by both "Republish" (when archived) and "Mark as not outdated" (when still published).
  const resolveOutdated = useMutation({
    mutationFn: async (articleId: string) => {
              ...
              <div key={f.id} className="border-t border-black/5 dark:border-white/10 pt-3 first:border-t-0 first:pt-0">
                ...
                {f.articles && (
                  <div className="flex gap-2 mt-2">
                    {isArchived ? (
                      <button
                        onClick={() => resolveOutdated.mutate(f.articles!.id)}
                      ...
                      <>
                        <button
                          onClick={() => unpublish.mutate(f.articles!.id)}
                        ...
                        <button
                          onClick={() => resolveOutdated.mutate(f.articles!.id)}
                          className="rounded-lg border border-signal-teal/30 text-signal-teal px-3 py-1.5 text-xs font-medium"
                        >
